...
from .utils import cookieCart, cartData, translate_the_text
from clickuz import ClickUz
from django.views.decorators.csrf import csrf_exempt
from PaycomUz.status import ORDER_FOUND, ORDER_NOT_FOND, INVALID_AMOUNT
# Create your views here.
from .models import Product, ProductServices
...
def store(request):
    ...
    news = data['all_news']
    context = {"products": products, 'productServices': productService, "cartItems": cartItems, "all_news": news, "text": text}
    return render(request, "new_design/index.html", context)
...
def products(request):
    ...
    products = Product.objects.all()
    news = data['all_news']
    context = {"text":text,"products": products, 'productServices': productService, "cartItems": cartItems, "all_news": news}
    return render(request, "new_design/all-products.html", context)


def product_services_detail(request, id):
    text = translate_the_text(request.LANGUAGE_CODE)
    data = cartData(request)
    cartItems = data['cartItems']
    productService = ProductServices.objects.get(id=id)
    productServices = ProductServices.objects.all()
    products = Product.objects.all()

    news = data['all_news']
    context = {"text":text,"products": products, 'productServices': productServices, 'productService': productService,
               "cartItems": cartItems, "all_news": news}
    return render(request, "new_design/erp-smarty.html", context)
...
def product_detail(request, id):
    ...
    productService = ProductServices.objects.all()
    news = data['all_news']
    context = {"text":text,"products": products, 'product': product, 'productServices': productService, "cartItems": cartItems,
               "all_news": news}
    return render(request, "new_design/product-data 2.html", context)
...
def cart(request):
    ...
    news = data['all_news']
    context = {"text":text,"products": products, "productServices": productServices, "order": order, 'items': items,
               "cartItems": cartItems, "all_news": news}
    return render(request, 'new_design/cart.html', context)
...
def checkout(request):
    ...
    context = {"text":text,"products": products, "productServices": productServices, "order": order, 'items': items,
               "cartItems": cartItems, "all_news": news}
    return render(request, 'new_design/checkout.html', context)
...
def processOrder(request):
    ...
    if request.user.is_authenticated:
        ...
        url = ClickUz.generate_url(order_id=f'{order.id}', amount=f'{order.get_cart_total}',
                                   return_url='success_payment')
